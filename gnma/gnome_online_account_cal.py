...
class GnomeOnlineAccountCal:
    ...
    def is_relevant_source(self, _, source):
        relevant = (
    ...
    def source_appeared(self, watcher, source):
        if (
            self.config["restrict_to_calendar"]
            and source.get_display_name() not in self.config["restrict_to_calendar"]
        ):
            logging.debug(
                "[SKIP] skipping calendar not in config: %s", source.get_display_name()
            )
            return
